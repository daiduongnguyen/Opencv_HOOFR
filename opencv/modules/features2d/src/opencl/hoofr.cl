// OpenCL port of the HOOFR feature detector and descriptor extractor
...
#ifdef HOOFR_RESPONSES

__kernel void
HOOFR_HarrisResponses(__global const uchar* imgbuf, int imgstep, int imgoffset0,
                    ...
                    __global float* responses, int nkeypoints )
{
    ...
    if( idx < nkeypoints )
    {
        ...
        responses[idx] = ((float)a * b - (float)c * c - HOOFR_HARRIS_K * (float)(a + b) * (a + b))*scale_sq_sq;
    }
}
...
__kernel void
HOOFR_ICAngle(__global const uchar* imgbuf, int imgstep, int imgoffset0,
            __global const int* layerinfo, __global const int* keypoints,
...
#ifdef HOOFR_DESCRIPTORS

__kernel void
HOOFR_computeDescriptor(__global const uchar* imgbuf, int imgstep, int imgoffset0,
                      __global const int* layerinfo, __global const int* keypoints,
